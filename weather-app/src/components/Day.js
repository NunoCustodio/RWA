...
const Day = ({ dayData }) => {
  ...
  var pic = weatherType.filter(
    (dayType) => dayType.idWeatherType === dayData.idWeatherType
  )[0];
  if (pic) {
    return (
      <>
        <img
          src={`${window.location}/assets/images/${pic.weatherPic}`}
          alt={pic.alt}
        />
        <p>{dayData.date}</p>
        <p>
          Min: {dayData.tMin} Max: {dayData.tMax}
        </p>
      </>
    );
  } else {
